...
const P = {
  ...
  cal:   "M8 2v4 M16 2v4 M3 10h18 M3 6a2 2 0 012-2h14a2 2 0 012 2v14a2 2 0 01-2 2H5a2 2 0 01-2-2V6z",
...
export default function App() {
  ...
  // 캘린더 상태
  const [calYear,  setCalYear]  = useState(YY);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      done=true; clearTimeout(timer); setLoaded(true);
    };
    load();
    return ()=>clearTimeout(timer);
  },[]);

  const sv = async (k,val) => { try{ await window.storage.set(k,val); }catch{} };
  useEffect(()=>{ if(loaded) sv("ad-collab", JSON.stringify(data)); },[data,loaded]);
  useEffect(()=>{ if(loaded) sv("ov-flw", flw); },[flw,loaded]);
  useEffect(()=>{ if(loaded) sv("ov-vi", JSON.stringify(vi)); },[vi,loaded]);
  ...
  const dateEvents = useMemo(()=>{
    ...
    data.forEach(r=>{
      addEv(r.contractDate, {type:"contract", company:r.company, r});
      addEv(r.deadline,     {type:"deadline", company:r.company, r});
    });
    return map;
  },[data]);

  const getDateStr = (d) => `${calYear}-${String(calMonth).padStart(2,"0")}-${String(d).padStart(2,"0")}`;
  const todayStr   = `${YY}-${String(MM).padStart(2,"0")}-${String(now.getDate()).padStart(2,"0")}`;
  const selEvents  = selDate ? (dateEvents[getDateStr(selDate)]||[]) : [];

  const prevMonth = () => { if(calMonth===1){setCalMonth(12);setCalYear(y=>y-1);}else setCalMonth(m=>m-1); setSelDate(null); };
  const nextMonth = () => { if(calMonth===12){setCalMonth(1);setCalYear(y=>y+1);}else setCalMonth(m=>m+1); setSelDate(null); };
  ...
  const iBtn = a => ({ width:40, height:40, borderRadius:10, background:a?"#3a3a3c":"transparent", color:a?"#fff":"#888", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", border:"none" });
    ...
    <div style={{ display:"flex", height:"100vh", fontFamily:"'Pretendard','Apple SD Gothic Neo',sans-serif", overflow:"hidden" }}>
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .cal-day{border-radius:10px;cursor:pointer;padding:6px 4px;text-align:center;min-height:68px;position:relative;}
        .cal-day:hover{background:#f7f7f5;}
        .cal-day.today .dn{background:#1c1c1e;color:#fff;border-radius:50%;width:26px;height:26px;display:flex;align-items:center;justify-content:center;margin:0 auto;}
        .cal-day.selected{background:#f0f0ec;}
        .cal-day.has-ev .dn{font-weight:700;}
      `}</style>
      ...
      <div style={{ width:60, background:"#1c1c1e", display:"flex", flexDirection:"column", alignItems:"center", paddingTop:16, paddingBottom:16, gap:8, flexShrink:0 }}>
        ...
        <div style={{ flex:1, display:"flex", flexDirection:"column", gap:4 }}>
          {[["dashboard",P.home],["contracts",P.file],["calendar",P.cal]].map(([k,ic])=>(
            <button key={k} style={iBtn(tab===k)} onClick={()=>setTab(k)}><Ico d={ic} size={18}/></button>
          ))}
        </div>
      ...
      <div style={{ width:220, background:"#f7f7f5", borderRight:"1px solid #e8e8e6", display:"flex", flexDirection:"column", flexShrink:0, overflowY:"auto" }}>
        ...
        {[
          { title:"메뉴", items:[
            ...
            { label:"캘린더",    active:tab==="calendar",  onClick:()=>setTab("calendar")  },
          ]},
          { title:"현황", items:[
      ...
      <div style={{ flex:1, overflowY:"auto", background:"#f0f0ec", padding:"28px" }}>
        {!loaded && <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:"100%", color:"#aaa", fontSize:13 }}>불러오는 중...</div>}
            ...
            <div style={card}>
              ...
              {editFlw ? (
                ...
                <div style={{ display:"flex", alignItems:"flex-end", justifyContent:"space-between" }}>
                  <div><span style={{ fontSize:32, fontWeight:800, color:"#1c1c1e", letterSpacing:"-1px" }}>{flw||"—"}</span>{flw&&<span style={{ fontSize:13, color:"#aaa", marginLeft:4 }}>명</span>}</div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1.1fr 0.9fr", gap:14, alignItems:"start" }}>
            <div style={card}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:18 }}>
                <div><div style={{ fontSize:15, fontWeight:700, color:"#1c1c1e" }}>월별 수입 그래프</div><div style={{ fontSize:12, color:"#aaa", marginTop:3 }}>막대 클릭 → 조회수 수입 입력</div></div>
                <div style={{ fontSize:12, color:"#aaa", background:"#f7f7f5", padding:"5px 12px", borderRadius:8 }}>연간 <strong style={{ color:"#1c1c1e" }}>{fmt(chartData.reduce((s,d)=>s+d.total,0))}</strong></div>
              </div>
              <ResponsiveContainer width="100%" height={230}>
                <ComposedChart data={chartData} margin={{ top:4,right:4,left:0,bottom:0 }} onClick={e=>{ if(e?.activePayload?.length){const d=e.activePayload[0]?.payload;if(d){setEditVM(d.mk);setViIn(String(d.va||""));}}}}>
            ...
            <div style={card}>
              <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16 }}>
                <div><div style={{ fontSize:15,fontWeight:700,color:"#1c1c1e" }}>진행 중인 계약</div><div style={{ fontSize:12,color:"#aaa",marginTop:3 }}>마감일이 지나지 않은 계약</div></div>
                  ...
                  return (
                    <div key={r.id} onClick={()=>{setTab("contracts");setDetail(r);}} style={{ padding:"12px 0",borderBottom:"1px solid #f7f7f5",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"flex-start" }}>
                      ...
                      <div style={{ textAlign:"right",flexShrink:0 }}>
                        {r.sponsored?<span style={{ background:"#f0f0ec",color:"#555",fontSize:11,fontWeight:700,padding:"3px 10px",borderRadius:20 }}>협찬</span>:<div style={{ fontSize:13,fontWeight:700,color:"#1c1c1e" }}>{fmt(r.amount)}</div>}
        ...
        {loaded && tab==="calendar" && (<>
          <div style={{ marginBottom:24 }}>
            <h1 style={{ fontSize:26, fontWeight:800, color:"#1c1c1e", letterSpacing:"-.5px" }}>캘린더</h1>
            <p style={{ fontSize:13, color:"#aaa", marginTop:4 }}>계약일 · 마감일을 한눈에</p>
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 300px", gap:16, alignItems:"start" }}>
            ...
            <div style={card}>
              ...
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
                <button style={{ ...btn, background:"#f0f0ec", color:"#555", padding:"7px 10px" }} onClick={prevMonth}><Ico d={P.left} size={16}/></button>
                <span style={{ fontSize:18, fontWeight:800, color:"#1c1c1e" }}>{calYear}년 {calMonth}월</span>
                <button style={{ ...btn, background:"#f0f0ec", color:"#555", padding:"7px 10px" }} onClick={nextMonth}><Ico d={P.right} size={16}/></button>
              </div>

              {/* 요일 헤더 */}
              <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", marginBottom:6 }}>
                {DAYS.map((d,i)=>(
                  <div key={d} style={{ textAlign:"center", fontSize:12, fontWeight:700, color: i===0?"#c62828":i===6?"#1a6aab":"#aaa", padding:"6px 0" }}>{d}</div>
                ))}
              </div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:2 }}>
                {calDays.map((d,i)=>{
                  ...
                  const evs = dateEvents[ds]||[];
                  const isToday = ds===todayStr;
                  const isSel   = selDate===d;
                  const hasContract = evs.some(e=>e.type==="contract");
                  const hasDeadline = evs.some(e=>e.type==="deadline");
                  const dow = new Date(calYear,calMonth-1,d).getDay();
                  return (
                    <div key={d} className={`cal-day${isToday?" today":""}${isSel?" selected":""}${evs.length?" has-ev":""}`}
                      onClick={()=>setSelDate(isSel?null:d)}
                      style={{ background: isSel?"#f0f0ec":"transparent" }}>
                      <div className="dn" style={{ fontSize:13, color: isToday?"":dow===0?"#c62828":dow===6?"#1a6aab":"#1c1c1e", fontWeight: evs.length?700:400, display:"flex", alignItems:"center", justifyContent:"center", width:26, height:26, margin:"0 auto", borderRadius:"50%", background: isToday?"#1c1c1e":"transparent" }}>{d}</div>
                      <div style={{ display:"flex", flexDirection:"column", gap:2, marginTop:4, padding:"0 2px" }}>
                        {hasContract && <div style={{ background:"#1c1c1e", borderRadius:4, fontSize:9, fontWeight:700, color:"#fff", padding:"1px 4px", textAlign:"center", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>계약</div>}
                        {hasDeadline && <div style={{ background:"#c62828", borderRadius:4, fontSize:9, fontWeight:700, color:"#fff", padding:"1px 4px", textAlign:"center", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>마감</div>}
                      </div>
                    </div>
                  );
                })}
              ...
              {/* 범례 */}
              <div style={{ display:"flex", gap:16, marginTop:16, paddingTop:14, borderTop:"1px solid #f0f0ec" }}>
                {[["#1c1c1e","계약일"],["#c62828","마감일"]].map(([c,l])=>(
                  <div key={l} style={{ display:"flex", alignItems:"center", gap:6, fontSize:12, color:"#888" }}>
                    <span style={{ width:10, height:10, borderRadius:3, background:c, display:"inline-block" }}></span>{l}
                  </div>
                ))}
                <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:12, color:"#888" }}>
                  <span style={{ width:22, height:22, borderRadius:"50%", background:"#1c1c1e", display:"inline-block" }}></span>오늘
                </div>
              </div>
            </div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
              {/* 선택 날짜 이벤트 */}
              <div style={card}>
                <div style={{ fontSize:14, fontWeight:700, color:"#1c1c1e", marginBottom:12 }}>
                  {selDate ? `${calMonth}월 ${selDate}일` : "날짜를 선택하세요"}
                </div>
                {!selDate && <div style={{ fontSize:13, color:"#ccc", textAlign:"center", padding:"20px 0" }}>캘린더에서 날짜를 클릭하면 해당 날짜의 계약이 표시돼요</div>}
                {selDate && selEvents.length===0 && <div style={{ fontSize:13, color:"#ccc", textAlign:"center", padding:"16px 0" }}>이 날짜에 일정이 없어요</div>}
                {selEvents.map((ev,i)=>(
                  <div key={i} onClick={()=>{setTab("contracts");setDetail(ev.r);}} style={{ padding:"10px 12px", borderRadius:10, marginBottom:8, cursor:"pointer", background: ev.type==="contract"?"#f7f7f5":"#fff5f5", borderLeft:`3px solid ${ev.type==="contract"?"#1c1c1e":"#c62828"}` }}>
                    <div style={{ fontSize:11, fontWeight:700, color: ev.type==="contract"?"#555":"#c62828", textTransform:"uppercase", letterSpacing:".04em", marginBottom:4 }}>{ev.type==="contract"?"계약일":"마감일"}</div>
                    <div style={{ fontSize:13, fontWeight:700, color:"#1c1c1e" }}>{ev.company}</div>
                    <div style={{ fontSize:11, color:"#aaa", marginTop:3, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{ev.r.content}</div>
                    {!ev.r.sponsored && <div style={{ fontSize:11, color:"#555", marginTop:4, fontWeight:600 }}>{fmt(ev.r.amount)}</div>}
                  </div>
                ))}
              </div>
              ...
              {/* 이번 달 마감 요약 */}
              <div style={card}>
                <div style={{ fontSize:14, fontWeight:700, color:"#1c1c1e", marginBottom:12 }}>{calMonth}월 마감 일정</div>
                {data.filter(r=>{ const[y,m]=r.deadline.split("-").map(Number); return y===calYear&&m===calMonth; }).length===0
                  ? <div style={{ fontSize:13, color:"#ccc", textAlign:"center", padding:"12px 0" }}>마감 일정이 없어요</div>
                  : data.filter(r=>{ const[y,m]=r.deadline.split("-").map(Number); return y===calYear&&m===calMonth; })
                      .sort((a,b)=>a.deadline>b.deadline?1:-1)
                      .map(r=>{
                        const days=daysLeft(r.deadline);
                        const col=days<0?"#aaa":days<=7?"#c62828":days<=30?"#bf6e00":"#2e7d32";
                        return (
                          <div key={r.id} onClick={()=>{setTab("contracts");setDetail(r);}} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"9px 0",borderBottom:"1px solid #f7f7f5",cursor:"pointer" }}>
                            <div>
                              <div style={{ fontSize:13,fontWeight:600,color:"#1c1c1e" }}>{r.company}</div>
                              <div style={{ fontSize:11,color:"#aaa",marginTop:2 }}>{r.deadline}</div>
                            </div>
                            <span style={{ fontSize:11,fontWeight:700,color:col,whiteSpace:"nowrap" }}>{days<0?`${Math.abs(days)}일 초과`:`${days}일 남음`}</span>
                          </div>
                        );
                      })
                }
        ...
        {loaded && tab==="contracts" && (<>
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:24 }}>
            ...
            <button style={{ ...btn,background:"#1c1c1e",color:"#fff",display:"flex",alignItems:"center",gap:6 }} onClick={openAdd}><Ico d={P.plus} size={14}/> 새 협업 추가</button>
          </div>
          <div style={{ display:"flex",gap:10,marginBottom:18,flexWrap:"wrap" }}>
            ...
            <select style={{ maxWidth:140 }} value={fPaid} onChange={e=>{setFPaid(e.target.value);setFType("all");}}>
          ...
          {detail && (
            <div style={{ ...card,marginTop:16 }}>
              <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18 }}>
                <div><div style={{ fontSize:15,fontWeight:700,color:"#1c1c1e" }}>계약 상세 내역</div><div style={{ fontSize:12,color:"#aaa",marginTop:2 }}>{detail.company}</div></div>
